...
from app.rag.memory_extraction import (
    ENTITY_TYPES, _EXTRACT_PROMPT, _normalize_name, _now,
    _parse_extract_json, _upsert_relation,
)
...
def _corpus(package: dict | None) -> str:
    package = package or {}
    parts = []
    for key in ("requirement", "research", "competitor_analysis", "strategy", "design", "presentation"):
...
def _upsert_entity(repo, product_id: str, name: str, entity_type: str, summary: str):
    ...
    if existing:
        repo.update_entity_merge(str(existing.id), new_alias=name,
                                 new_summary=summary or existing.summary,
                                 confidence_delta=0.1, last_seen=_now())
        return existing.id
    return repo.save_memory_entity(
        scope="project", studio_product_id=product_id, name=name,
        type=entity_type, summary=summary, confidence=0.6,
...
def _vectorize(repo, entity_ids: list[str], product_id: str) -> None:
    try:
        ...
        for entity_id in entity_ids:
            ...
            if entity:
                rows.append({"content": f"{entity.name}：{entity.summary or ''}".strip("："),
                             "url": f"memory://entity/{entity_id}"})
        for insight in repo.list_insights(scope="project", studio_product_id=product_id, limit=20):
            rows.append({"content": insight.content, "url": f"memory://insight/{insight.id}"})
        if rows:
...
def extract_memory_from_studio_product(product_id: str, package: dict | None, llm):
    ...
    try:
        raw = llm.complete_json([
            {"role": "system", "content": "你是知识沉淀引擎。严格输出 JSON，不要 Markdown。"},
    ...
    for item in data.get("entities") or []:
        ...
        if entity_type not in ENTITY_TYPES:
            entity_type = "other"
        entity_id = _upsert_entity(repo, product_id, name, entity_type,
    ...
    for item in data.get("relations") or []:
        ...
        if _upsert_relation(repo, entity_ids[source], entity_ids[target],
                            str(item.get("relation") or "相关")[:50], product_id):
            saved_relations += 1

    existing_insights = {i.content for i in repo.list_insights(
    ...
    for item in data.get("insights") or []:
        ...
        if not content or content in existing_insights:
            continue
        linked = [eid for name, eid in entity_ids.items() if len(name) >= 2 and name in content][:10]
        repo.save_memory_insight(scope="project", studio_product_id=product_id,
                                 content=content, entity_ids=linked,
    ...
    result = {"product_id": product_id, "entities": len(saved_entities),
              "relations": saved_relations, "insights": saved_insights,
              "promoted": promoted}
...
def promote_global_studio_memories(repo, product_id: str) -> int:
    ...
    for entity in repo.list_studio_entities(product_id):
        ...
        if global_entity:
            ...
            repo.update_entity_merge(str(global_id),
                                     new_summary=entity.summary or global_entity.summary,
                                     confidence_delta=0.05, last_seen=_now())
        else:
            global_id = repo.save_memory_entity(
                scope="global", name=entity.name, type=entity.type,
                summary=entity.summary, confidence=min(0.9, entity.confidence + 0.1),
                first_seen=_now(), last_seen=_now(),
            ).id
        promoted += 1
        for relation in repo.list_relations_for_entity(str(entity.id), active_only=True):
            other_id = (relation.target_entity_id if str(relation.source_entity_id) == str(entity.id)
                        else relation.source_entity_id)
            source_id, target_id = ((global_id, other_id) if str(relation.source_entity_id) == str(entity.id)
                                    else (other_id, global_id))
            if not repo.find_relation(str(source_id), str(target_id), relation.relation_type):
                repo.save_memory_relation(source_id=str(source_id), target_id=str(target_id),
                                         relation_type=relation.relation_type, evidence=relation.evidence,
                                         weight=relation.weight, valid_from=relation.valid_from)
    return promoted
