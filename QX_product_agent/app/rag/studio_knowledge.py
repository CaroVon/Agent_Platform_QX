...
def sync_studio_knowledge(
    ...
    *,
    ...
    chunk_rows = [
        {"content": chunk, "url": f"studio://product/{product_id}"}
    ...
    tags = [
        str(word) for values in keywords.values() if isinstance(values, list)
        for word in values[:4]
    ]
    title = idea or package.get("idea") or f"Product Studio 任务 {product_id[:8]}"
    ProjectRepo().save_knowledge_asset(
        scope=f"studio:{product_id}", source="studio", title=title,
        source_url=f"studio://product/{product_id}", tags=tags[:20],
        chunk_count=len(chunk_rows), studio_product_id=product_id,
        extra={"product_id": product_id, "asset_keys": list(package.keys())},
    )
    logger.info("[Studio Knowledge] 任务知识已同步 | product=%s | chunks=%d", product_id, len(chunk_rows))
    return {"product_id": product_id, "chunks": len(chunk_rows), "assets": 1}
