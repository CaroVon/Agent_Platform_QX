...
def _full_deck() -> dict:
    """8 页完整演示（通过质量门）。"""
    return _presentation().model_dump()
...
def test_quality_gate_penalty_forces_revision():
    """质量门 error（metric 缺 value + chart 缺数据）压分 → 触发修订。"""
    bad_deck = _full_deck()
    # 制造两个 gate error：p2 metric 缺 value；p3 chart 缺数据
    bad_deck["pages"][1]["components"][0]["data"].pop("value")
    bad_deck["pages"][2]["components"][0]["data"].pop("items")
